...
安裝完畢後會因為缺乏環境路徑，可採用以下兩種方式運行：

+ 可離開 WSL 後再次啟動 ```wsl-env.bat``` 進入環境，即可使用
+ 執行 ```source $HOME/.local/bin/env``` 設定環境路徑，即可使用

設計 WSL 環境，主要運用 Claude 用戶登入的方案。
...
若使用 Docker 啟動容器常駐登入狀態，可採用 Claude 訂閱方案方式執行；原則上，此方式等同於本機安裝，只是透過容器與主機隔離。

其操作方式如下：

1. 執行 ```do.bat srv``` 啟動服務，並自動進入 Claude CLI 的互動介面。
2. 選擇 ```Claude account with subscription```。
3. 取得認證用網址，前往可認證該網址的瀏覽器。
4. 選擇帳戶，並取得驗證碼
5. 回到 Claude CLI 互動介面複製驗證碼，等候服務啟動完畢

執行詢問方式如下：

1. 執行 ```docker exec -ti claude-cli-srv bash``` 進入容器
2. 執行 ```claude --print "What is claude"``` 詢問內容
